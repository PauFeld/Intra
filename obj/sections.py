...
from vedo import *
filename = 'ArteryObjAN1-4.vtp'
msh = load(filename)
msh.cmap('viridis', msh.points()[:,1])
#msh.show()

centerline = read_vtk("centerlines/ArteryObjAN1-4-network.vtp")
...
for i in range (numberOfCellPoints):
    ...
    plane = Grid( pos = centerline_np[i, :3],s = (10,10), res = (100,100), normal = tangent).wireframe(False)
...
np.save('radius/ArteryObjAN1-4-radiusbis.npy', radius_array)
...
plane = Grid( pos = centerline_np[10, :3], s = (10, 10), res = (150,150),normal = tangent).wireframe(False)
